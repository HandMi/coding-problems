...
class CF0618D_CalculateDegreeTest
    : public testing::TestWithParam<std::pair<Tree, int>>
{
};

TEST_P(CF0618D_CalculateDegreeTest, CalculateDegree)
{
...
INSTANTIATE_TEST_CASE_P(
    CF0618D_CalculateDegreeTests, CF0618D_CalculateDegreeTest,
    ::testing::Values(std::make_pair(Tree{{1},{0}}, 1),
                      std::make_pair(Tree{{2},{2},{0, 1, 3}, {2, 4, 5}, {3}, {3}}, 3),
                      std::make_pair(Tree{{2},{2},{0, 1, 3, 4, 5}, {2}, {2}, {2}}, 5)));
// clang-format on

class CF0618D_CountUseableEdgesTest
    : public testing::TestWithParam<std::pair<Tree, int>>
{
};

TEST_P(CF0618D_CountUseableEdgesTest, CountUseableEdges)
{
    auto tree = GetParam().first;
    auto expected = GetParam().second;
...
// clang-format off
INSTANTIATE_TEST_CASE_P(
    CF0618D_CountUseableEdgesTests, CF0618D_CountUseableEdgesTest,
    ::testing::Values(std::make_pair(Tree{{1},{0}}, 1),
